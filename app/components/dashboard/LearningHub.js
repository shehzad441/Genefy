// components/dashboard/LearningHub.js
...
export default function LearningHub() {
  const cards = [
    ...
    { title: 'Study Notes', color: 'bg-teal-500', icon: '/images/notes.png' },
    ...
    <div className="mt-8">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {cards.map((card, index) => (
          <DashboardCard key={index} title={card.title} color={card.color} icon={card.icon} />
        ))}
      </div>
    </div>
  );
